...
struct generator {
    ...
    static void handle_contract() noexcept {
        for (const auto& pair : contract::elements) {
            if (auto msg = dynamic_pointer_cast<message>(pair.second)) {
                write_to_file("msg_srpc.hpp", handle_message(msg));
            } else if (auto svc = dynamic_pointer_cast<service>(pair.second)) {
                write_to_file("msg_srpc.hpp", handle_service(svc));
            }
        }
    }
    ...
    [[nodiscard]] static std::string handle_nested_message_field(const std::string& type, const std::string& name) noexcept {
        std::string msg_gen;
        msg_gen += "\t\t" + type + " " + name + "_;\n";
        msg_gen += "\t\t" + name + "_.unpack(packed, offest);\n";
    ...
    static signed write_to_file(const std::string& file_name, const std::string& s) noexcept {
