...
struct generator {
    ...
    [[nodiscard]] static std::string handle_service(std::shared_ptr<service> svc) noexcept {
        std::string stub_msg_gen = "struct " + svc->name + "_stub {\n";
        for (const auto& m: svc->methods()) {
            stub_msg_gen += "\t" + m->output_t + " " + m->name + "(const " + m->input_t + "& req) {\n";
            stub_msg_gen += "\t\tstd::vector<uint8_t> packed = srpc::packer::pack(req);\n"; 
            stub_msg_gen += "\t}\n";
        }
        stub_msg_gen += "};\n\n";  

        std::string servicer_msg_gen = "struct " + svc->name + "_servicer {\n";
        for (const auto& m: svc->methods()) {
            servicer_msg_gen += "\tvirtual " + m->output_t + " " + m->name + "(const " + m->input_t + "& req) = 0;\n";
        }
        servicer_msg_gen += "};\n\n";
    ...
    static signed init_file(const std::string& file_path) noexcept {
        ...
        init_msg += "#include <srpc/packer.hpp>\n";
        init_msg += "#include <vector>\n";
